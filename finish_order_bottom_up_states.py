...
def print_ordering_bounds(ordering_bounds):
...
def bottom_up_dp_from_state(states, m):
    ...
    Returns a tuple:
      (final_results, intermediate_states)
      
      - final_results: dict mapping complete finish_order -> finish_intervals,
        for states where waiting and running are empty.
      - intermediate_states: a list of states that have not reached completion.
    """
    seen = set()
    final_results = {}
    # Worklist for DP states.
    ...
    while worklist:
        ...
        for state in worklist:
            ...
            # If there are waiting tasks and free cores, dispatch the next waiting task.
            ...
            # Otherwise, process the next finish event.
            elif running:
                ...
                if state_key not in seen:
                    seen.add(state_key)
                    next_states.append(new_state)
        # Collect states that are still not final.
        for s in next_states:
            if s[1] or s[2]:
                intermediate.append(s)
        worklist = next_states

    return final_results, intermediate
...
def extend_dp_states(intermediate_states, new_task, m):
    ...
    This will compute the complete finish orderings for dispatch order x+1,
    reusing the work already done in the intermediate states.
    
    Returns:
      final_results: dict mapping finish_order -> finish_intervals
    """
    # For each intermediate state, add the new_task to the waiting list.
    extended_states = []
...
def main():
    ...
    # tasks = {
    #     {"t1": {"ft": (1, 3),   "exec_range": (1, 3)}},
    #     {"t6": {"ft": (5, 11),   "exec_range": (5, 11)}},
    #     {"t9": {"ft": (11, 23),   "exec_range": (10, 20)}},
    #     {"t2": {"ft": (6, 14),   "exec_range": (1, 3)}},
    ...
    final_results_x, inter = bottom_up_dp_from_state([initial_state], m=2)
    print_ordering_bounds(final_results_x)
    print("[6, 14], [11, 23]")
    res, inter = extend_dp_states(inter, 't3', m=2)
    print_ordering_bounds(res)
    print("[8, 18], [11, 23]")
    res, inter = extend_dp_states(inter, 't7', m=2)
    print_ordering_bounds(res)
    print("[11, 23], [14, 30]")
    res, inter = extend_dp_states(inter, 't4', m=2)
    print_ordering_bounds(res)
    print("[13, 28], [14, 30]")
